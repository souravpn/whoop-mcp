...
if (!clientId || !clientSecret) {
  process.stderr.write(
    '[whoop-mcp] Error: WHOOP_CLIENT_ID and WHOOP_CLIENT_SECRET are required.\n' +
...
function safe(fn: () => Promise<string>): Promise<{ content: { type: 'text'; text: string }[] }> {
  return fn()
...
server.tool(
  'get_recovery',
  "Get the user's most recent WHOOP recovery score, HRV, resting heart rate, and SpO2",
  {},
  () => safe(async () => {
    const recovery = await client.getLatestRecovery();
    if (!recovery) return 'No recovery data available yet. Make sure your WHOOP is synced.';
...
server.tool(
  'get_sleep',
  "Get the user's most recent WHOOP sleep data including duration, sleep stages (light, deep/SWS, REM), efficiency, and respiratory rate",
  {},
  () => safe(async () => {
    const sleep = await client.getLatestSleep();
    if (!sleep) return 'No sleep data available. Make sure your WHOOP is synced.';
...
server.tool(
  'get_strain',
  "Get the user's current day strain score, average heart rate, and calories burned from WHOOP",
  {},
  () => safe(async () => {
    const cycle = await client.getLatestCycle();
    if (!cycle) return 'No strain data available yet today.';
...
server.tool(
  'get_latest_workout',
  "Get the user's most recent WHOOP workout — sport type, duration, strain, heart rate zones, and calories",
  {},
  () => safe(async () => {
    const workout = await client.getLatestWorkout();
    if (!workout) return 'No recent workouts found.';
...
server.tool(
  'get_recovery_trend',
  "Get the user's WHOOP recovery scores over the past N days (default 7) for trend analysis",
  { days: z.number().min(1).max(25).default(7).describe('Number of days to look back (max 25)') },
  ({ days }) => safe(async () => {
    const start = new Date();
    start.setDate(start.getDate() - days);
    const records = await client.getRecoveryCollection(start.toISOString(), undefined, days);
    if (records.length === 0) return 'No recovery data found for this period.';
...
server.tool(
  'get_sleep_trend',
  "Get the user's WHOOP sleep data over the past N days (default 7) for trend analysis",
  { days: z.number().min(1).max(25).default(7).describe('Number of days to look back (max 25)') },
  ({ days }) => safe(async () => {
    const start = new Date();
    start.setDate(start.getDate() - days);
    const records = await client.getSleepCollection(start.toISOString(), undefined, days);
    if (records.length === 0) return 'No sleep data found for this period.';
...
server.tool(
  'get_workout_history',
  "Get the user's recent WHOOP workout history — useful for understanding training load and patterns",
  { count: z.number().min(1).max(25).default(5).describe('Number of workouts to retrieve (max 25)') },
  ({ count }) => safe(async () => {
...
server.tool(
  'get_profile',
  "Get the user's WHOOP profile (name, email) and body measurements (height, weight, max heart rate)",
  {},
  () => safe(async () => {
